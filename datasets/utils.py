...
def split_ssl_data(args, data, targets, num_classes,
                   ...
                   lb_index=None, ulb_index=None, include_lb_to_ulb=True, load_exist=True, labeled_percentage=0.1):
    """
    ...
    #                                                lb_imbalance_ratio, ulb_imbalance_ratio,labeled_percentage, load_exist=False)
    
    # manually set lb_idx and ulb_idx, do not use except for debug

    lb_idx, ulb_idx = sample_imb_data (args, targets, labeled_percentage, load_exist=False)
...
def sample_imb_data (args, targets, labeled_percentage, load_exist=True):

    dump_dir = os.path.join(base_dir, 'data', args.dataset, 'labeled_idx')
    os.makedirs(dump_dir, exist_ok=True)
    lb_dump_path = os.path.join(dump_dir, f'lb_labels{args.num_labels}_{args.lb_imb_ratio}_seed{args.seed}_imb_idx.npy')
    ulb_dump_path = os.path.join(dump_dir, f'ulb_labels{args.num_labels}_{args.ulb_imb_ratio}_seed{args.seed}_imb_idx.npy')
    ...
    _, lb_idx, _, _ = train_test_split(
        np.arange(len(targets)), targets, test_size=labeled_percentage, stratify=targets, random_state=42
    )

    ulb_idx = np.setdiff1d(np.arange(len(targets)), lb_idx)
...
def sample_labeled_unlabeled_data(args, data, target, num_classes,
                                  lb_num_labels, ulb_num_labels=None,
                                  lb_imbalance_ratio=1.0, ulb_imbalance_ratio=1.0,labeled_percentage=0.1,
                                  load_exist=True):
    '''
    ...
    if os.path.exists(lb_dump_path) and os.path.exists(ulb_dump_path) and load_exist:
        print ("ulb-lb files read not made")
        lb_idx = np.load(lb_dump_path)
        ulb_idx = np.load(ulb_dump_path)
        return lb_idx, ulb_idx 


    if labeled_percentage:
        print ("labeled_percentage:",labeled_percentage)
        lb_samples_per_class = np.floor(np.bincount(target) * labeled_percentage).astype(int)
        ulb_samples_per_class = np.bincount(target) - lb_samples_per_class
